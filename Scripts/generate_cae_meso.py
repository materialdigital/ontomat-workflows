...
if __name__ == "__main__":
    ...
    material_coefficients_path = input_args[-1]
    workflow_input_path = input_args[-2]
    
    with open(material_coefficients_path) as f:
        mc = json.load(f)
    
    with open(workflow_input_path) as f:
        workflow_input = json.load(f)

    orientations = workflow_input["meso"]["orientations"]
    ...
    cae_name = str(workflow_input["simulation"]["meso"]["cae_name"])
    job_name = str(workflow_input["simulation"]["meso"]["job_name"])
    
    """
    with open("output3.txt", "w") as f:
        f.write("{}".format(material_coefficients))
    """
    
    """
    
    /isi/programs/simulia/2023_1/Commands/abaqus cae noGUI=generate_cae_meso.py -- "/home/chri/w/OntOMat/Workflow/OntOMat_Workflow/Scripts/workflow_input_unix.json" "/home/chri/w/OntOMat/Workflow/Simulation/UD_hex_homogen_homogenized_stiffness_voigt.json"
